...
${venta.redondeo !== 0
  ? `<div class="row sm" style="color:#555"><span>Redondeo COP:</span><span class="amt">${venta.redondeo > 0 ? '+' : '-'}${fmt(Math.abs(venta.redondeo))}</span></div>`
  : ''}
${ivaTotal > 0
...
export default function POS() {
  ...
  const [ordenNum]                          = useState(() => String(Math.floor(Math.random() * 9000) + 1000))
  ...
  const { data: clientesLista = [] } = useQuery<{ id: string; nombre: string; telefono?: string }[]>({
    ...
    enabled:   metodoPago === 'credito' && showNumPad,
    staleTime: 60_000,
  })
  ...
  const removeItem = (key: string) =>
    setCart(prev => {
      ...
      if (next.length === 0) { setShowNumPad(false); setEfectivo('') }
      return next
    })

  const clearCart = () => {
    setCart([]); setEfectivo(''); setVentaOk(null); setShowNumPad(false)
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      // Números 0-9 → alimentar el numpad solo si ya está en vista pago con efectivo
      if (/^[0-9]$/.test(key) && showNumPad && metodoPago === 'efectivo') {
        e.preventDefault()
        if (key !== '0' || efectivo !== '') setEfectivo(v => v + key)
        return
      }
      // Backspace → borrar último dígito del numpad
      if (key === 'Backspace' && showNumPad && metodoPago === 'efectivo') {
        e.preventDefault()
        setEfectivo(v => v.slice(0, -1))
        return
      }
      // Delete / Supr → limpiar numpad
      if (key === 'Delete' && showNumPad && metodoPago === 'efectivo') {
        e.preventDefault()
        setEfectivo('')
        return
      }
      ...
      // Escape → cerrar numpad / limpiar búsqueda
      if (key === 'Escape') {
        if (busqueda) { setBusqueda(''); return }
        if (showNumPad) { setShowNumPad(false); setEfectivo('') }
      }
    }
    window.addEventListener('keydown', handleKey)
    return () => window.removeEventListener('keydown', handleKey)
  }, [cart, efectivo, showNumPad, canCobrar, isPending, busqueda, cobrar, metodoPago])
    ...
    <div className={cn(
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-white/5 flex-shrink-0 flex items-center justify-between">
          <div>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest leading-none">Orden</p>
            <p className="text-white font-bold text-xl leading-tight">#{ordenNum}</p>
          </div>
          ...
          {showNumPad && !ventaOk && (
            <button
              onClick={() => { setShowNumPad(false); setEfectivo(''); if (isMobile) setPosView('carrito') }}
              className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white
                         bg-white/5 hover:bg-white/10 px-3 py-2 rounded-xl transition-colors
                         active:scale-[0.95] select-none border border-white/5">
        ...
        {!showNumPad && !ventaOk && (
          <>
            {/* Lista de ítems — ocupa todo el espacio disponible */}
            ...
            {/* Totales + botón Cobrar (fijo abajo) */}
            <div className="border-t border-white/5 bg-[#0D1B2A] p-4 space-y-2.5 flex-shrink-0">
              {cart.length > 0 && (
                <>
                  ...
                  <button
                    onClick={() => { setShowNumPad(true); if (isMobile) setPosView('carrito') }}
                    className="w-full bg-[#EA580C] hover:bg-[#C2410C] text-white font-bold
                               rounded-xl py-4 text-lg transition-colors active:scale-[0.97] select-none
              ...
              {cart.length === 0 && (
                <p className="text-center text-gray-600 text-xs py-2">Orden vacía</p>
              )}
            </div>
          </>
        )}
        ...
        {showNumPad && !ventaOk && (
          <div className="flex-1 flex flex-col p-3 gap-2 overflow-y-auto">
            ...
            <div className="bg-brand-dark rounded-xl p-3 border border-white/5">
              ...
              {redondeo !== 0 && (
                <div className="flex justify-between text-[10px] mb-1">
                  <span className="text-gray-600">Subtotal {fmt(subTotal)}</span>
                  <span className={redondeo > 0 ? 'text-orange-400/80' : 'text-green-400/80'}>
                    Redondeo {redondeo > 0 ? '+' : ''}{fmt(redondeo)}
                  </span>
                </div>
              )}
              <div className="space-y-0.5">
                {cart.slice(0, 3).map(i => (
                  <p key={i.producto.id} className="text-[10px] text-gray-600 truncate">
                    {i.cantidad}× {i.producto.nombre}
                  </p>
                ))}
                {cart.length > 3 && (
                  <p className="text-[10px] text-gray-700">+{cart.length - 3} más…</p>
                )}
              </div>
            </div>
            ...
            {metodoPago === 'exacto' && (
              <div className="bg-[#162C50] border border-[#EA580C]/20 rounded-xl p-5 text-center space-y-2">
                <Zap size={26} className="text-[#EA580C] mx-auto" />
                <p className="text-xs text-gray-400">Total exacto a cobrar</p>
                <p className="text-3xl font-bold text-white tabular-nums">{fmt(total)}</p>
            ...
            {metodoPago === 'transferencia' && (
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5 text-center space-y-2">
                <Smartphone size={26} className="text-blue-400 mx-auto" />
                <p className="text-white font-bold text-2xl tabular-nums">{fmt(total)}</p>
            ...
            {metodoPago === 'credito' && (
              <div className="space-y-2">
                ...
                <div className="bg-[#162C50] border border-white/5 rounded-xl px-3 py-2.5 text-center">
                  <p className="text-xs text-gray-400">Total a crédito</p>
            ...
            <button
              disabled={!canCobrar || isPending}
              onClick={() => cobrar()}
              className={cn(
                'w-full rounded-xl py-4 text-lg font-bold transition-all duration-150 select-none mt-auto',
                canCobrar && !isPending
        ...
        {ventaOk && (
          <div className="flex-1 flex flex-col items-center justify-center p-4 gap-3">
            <div className={cn(
              'rounded-2xl p-6 text-center w-full border',
              ventaOk.metodo === 'credito'
                ? 'bg-pink-500/10 border-pink-500/20'
                : ventaOk.metodo === 'transferencia'
